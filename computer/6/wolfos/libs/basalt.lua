...
for b_c,c_c in pairs(a_c)do for d_c,_ac in pairs(c_c(cba))do b_b[d_c]=_ac end end end;return cba end;aa["plugins"]={}
...
return
{basalt=function(cb)local db;local _c;local ac
local bc;local cc
local function dc()if(db==nil)then db=cb.getMainFrame()end;local _d=16;local ad=6
local bd=99;local cd=99;local dd,__a=db:getSize()
...
if(cc>1)then
dc=ac(cc-1,ad:getText(),ad:getForeground(),ad:getBackground()or colors.lightGray)if(ad:getAutoSize())then
ad:getBase():setSize(#dc[1][1],#dc[1]-1)end end;return ad end,getFontSize=function(ad)return
cc end,draw=function(ad)bc.draw(ad)
...
db:addForegroundBox(2,ac+1,_c,1,bb)end end end)end}return cb end}end;aa["libraries"]={}
...
aa["libraries"]["basaltMon"]=function(...)local ab,bb=math.max,string.sub
return
function(cb)local db={}
local _c,ac=0,0;local bc,cc=1,1;local dc,_d=colors.black,colors.white
for __a,a_a in pairs(cb)do db[__a]={}for b_a,c_a in pairs(a_a)do
db[__a][b_a]={name=c_a,monitor=peripheral.wrap(c_a)}end end
local function ad()local __a,a_a=0,0;local b_a=0
for c_a,d_a in pairs(db)do local _aa=0
for aaa,baa in pairs(d_a)do
local caa,daa=baa.monitor.getSize()_aa=_aa+caa;b_a=ab(b_a,daa)end;__a=ab(__a,_aa)a_a=a_a+b_a;b_a=0 end;_c,ac=__a,a_a end;ad()
local function bd(__a,a_a)local b_a,c_a=0,0;local d_a=0
for _aa,aaa in pairs(db)do local baa=0
for caa,daa in pairs(aaa)do
local _ba,aba=daa.monitor.getSize()baa=baa+_ba;d_a=ab(d_a,aba)
if(
__a>=b_a and __a<=b_a+_ba and a_a>=c_a and a_a<=c_a+aba)then return daa.monitor,caa,_aa end;b_a=b_a+_ba end;b_a=0;c_a=c_a+d_a;d_a=0 end end
local function cd(__a,a_a,b_a)local c_a,d_a=0,0;local _aa=0
for aaa,baa in pairs(db)do local caa=0
for daa,_ba in pairs(baa)do
local aba,bba=_ba.monitor.getSize()caa=caa+aba;_aa=ab(_aa,bba)
if(_ba.monitor==__a)then return a_a-c_a,b_a-d_a end;c_a=c_a+aba end;c_a=0;d_a=d_a+_aa;_aa=0 end end
local function dd(__a,a_a,b_a)local c_a,d_a,_aa=bd(bc,cc)
for aaa,baa in pairs(db[_aa])do
if(aaa>=d_a)then
local caa,daa=cd(baa.monitor,bc,cc)local _ba,aba=baa.monitor.getSize()local bba=bb(__a,1,_ba)
__a=bb(__a,_ba+1)local cba=bb(a_a,1,_ba)a_a=bb(a_a,_ba+1)local dba=bb(b_a,1,_ba)
b_a=bb(b_a,_ba+1)baa.monitor.setCursorPos(caa,daa)
baa.monitor.blit(bba,cba,dba)bc=bc+_ba end end end
return
{getSize=function()return _c,ac end,blit=dd,getCursorPos=function()return bc,cc end,setCursorPos=function(__a,a_a)bc,cc=__a,a_a;for b_a,c_a in pairs(db)do
for d_a,_aa in pairs(c_a)do
local aaa,baa=cd(_aa.monitor,bc,cc)_aa.monitor.setCursorPos(aaa,baa)end end end,getCursorBlink=function()
local __a=bd(bc,cc)return __a.getCursorBlink()end,setCursorBlink=function(__a)for a_a,b_a in pairs(db)do
for c_a,d_a in
pairs(b_a)do d_a.monitor.setCursorBlink(__a)end end end,setBackgroundColor=function(__a)
dc=__a end,getBackgroundColor=function()return dc end,setTextColor=function(__a)_d=__a end,getTextColor=function()return _d end,calculateClick=function(__a,a_a,b_a)local c_a=0
for d_a,_aa in
pairs(db)do local aaa=0;local baa=0
for caa,daa in pairs(_aa)do local _ba,aba=daa.monitor.getSize()if
(daa.name==__a)then return a_a+aaa,b_a+c_a end;aaa=aaa+_ba;if(aba>baa)then
baa=aba end end;c_a=c_a+baa end;return a_a,b_a end}end end
...
aa["objects"]["Object"]=function(...)
local ab=da("basaltEvent")local bb=da("utils")local cb=bb.splitString;local db=bb.uuid
local _c,ac=table.unpack,string.sub
return
...
type(_aa)=="function")then aaa=true end;if(c_a=="string")and(type(_aa)~="function")then
_aa=tostring(_aa)aaa=true end
if(not aaa)then local baa=type(_aa)
if(
type(c_a)=="table")then c_a=table.concat(c_a,", ")baa=_aa end
error(d_a:getType()..": Invalid type for property "..bc..
...
cc:setType("MonitorFrame")local dc=false
cc:addProperty("Monitor","string|table",nil,false,function(ad,bd)if(type(bd)=="string")then
local cd=peripheral.wrap(bd)if(cd~=nil)then ad:setTerm(cd)end elseif(type(bd)=="table")then
ad:setTerm(bd)end end)
...
table.insert(db,bc)ac:updateDraw()end;while
#db>ac:getMaxEntries()do table.remove(db,1)end;return ac end,clear=function(ac)
...
local _d=cc:getAutoSize()dc=tostring(dc)
...
table.insert(cd,d_a)_c:updateDraw()return d_a end,setExpanded=function(a_a,b_a)
ad=b_a;_c:updateDraw()return dd end,isExpanded=function(a_a)return ad end,onSelect=function(a_a,...)
for b_a,c_a in
pairs(table.pack(...))do if(type(c_a)=="function")then __a=c_a end end;return dd end,callOnSelect=function(a_a)if(__a~=nil)then
__a(dd)end end,setExpandable=function(a_a,b_a)_d=b_a;_c:updateDraw()return dd end,isExpandable=function(a_a)return
_d end,removeChild=function(a_a,b_a)
if(type(b_a)=="table")then for c_a,d_a in pairs(b_a)do
if(d_a==b_a)then b_a=c_a;break end end end;table.remove(cd,b_a)_c:updateDraw()return dd end,findChildrenByText=function(a_a,b_a)
local c_a={}
...
local b_a=a_a=="left"and-__a:len()or 3;bc:addText(b_a,_d,__a)end end)end}ac.__index=ac;return setmetatable(ac,_c)end end
...
_c:addTextBox(1,1,math.ceil(ac/100 *cc.Progress),bc,_d)end end)end}db.__index=db;return setmetatable(db,cb)end end;return aa["main"]()